...
public class macroupperrightarmhm extends BaseHullMod {
	...
	private static org.apache.log4j.Logger log = Global.getLogger(macroupperrightarmhm.class);
...
//		ShipAPI ship = weapon.getShip();
		...
		if (this.ship.isHulk() || !this.ship.isAlive()){
			return;
		}
		...
		if(interval.intervalElapsed() && arm.getSlot() != null && forearm.getSlot() != null && hand.getSlot() != null) {
			...
			Vector2f armabsloc = VectorUtils.rotateAroundPivot(new Vector2f(arm.getSlot().getLocation().getX() + this.ship.getLocation().getX(), arm.getSlot().getLocation().getY() + this.ship.getLocation().getY()), this.ship.getLocation(), this.ship.getFacing());
			if(target != null){
				...
				if (AIUtils.getNearestEnemy(this.ship) != null) {
					for(ShipAPI e:AIUtils.getNearbyEnemies(this.ship,1000f)){
				...
				if (AIUtils.getNearbyEnemyMissiles(this.ship, index) != null) {
					for (MissileAPI m : AIUtils.getNearbyEnemyMissiles(this.ship, 1000f)) {
			...
			if(targloc == null) {
				if (arm != null) {
					...
					float dangle = (3600f - 49.5f + this.ship.getFacing()) % 360f;
				...
				if (forearm != null) {
					forearm.getSlot().getLocation().set(VectorUtils.rotateAroundPivot(new Vector2f(arm.getSlot().getLocation().getX() + 110f, arm.getSlot().getLocation().getY()), arm.getSlot().getLocation(), arm.getCurrAngle() - this.ship.getFacing()));

					float dangle = (3600f + 22.5f + this.ship.getFacing()) % 360f;
				...
				if (arm != null) {
					Vector2f slotabsloc = VectorUtils.rotateAroundPivot(new Vector2f(arm.getSlot().getLocation().getX() + this.ship.getLocation().getX(), arm.getSlot().getLocation().getY() + this.ship.getLocation().getY()), this.ship.getLocation(), this.ship.getFacing());
					...
					float dangle = (3600f + MathUtils.clamp(VectorUtils.getAngle(slotabsloc, targloc) + 90f - targangle, -10f + this.ship.getFacing(), 110f + this.ship.getFacing())) % 360f;
					float currangle = (3600f + arm.getCurrAngle()) % 360f;
				...
				if (forearm != null) {
					...
					Vector2f slotabsloc = VectorUtils.rotateAroundPivot(new Vector2f(forearm.getSlot().getLocation().getX() + this.ship.getLocation().getX(), forearm.getSlot().getLocation().getY() + this.ship.getLocation().getY()), this.ship.getLocation(), this.ship.getFacing());
					float dist = MathUtils.clamp(MathUtils.getDistance(slotabsloc, targloc), 0f, 220f) * 0.5f;
			...
			if (hand != null) {
				hand.getSlot().getLocation().set(VectorUtils.rotateAroundPivot(new Vector2f(forearm.getSlot().getLocation().getX() + 110f, forearm.getSlot().getLocation().getY() - 4f), forearm.getSlot().getLocation(), forearm.getCurrAngle() - this.ship.getFacing()));

				Vector2f slotabsloc = VectorUtils.rotateAroundPivot(new Vector2f(hand.getSlot().getLocation().getX() + this.ship.getLocation().getX(), hand.getSlot().getLocation().getY() + this.ship.getLocation().getY()), this.ship.getLocation(), this.ship.getFacing());
				if(target != null){
					if(MathUtils.getDistance(slotabsloc,target.getLocation())<10f){
						while (target.getHullLevel()>0){
							engine.applyDamage(target, target.getLocation(), 100f, DamageType.FRAGMENTATION, 0, true, false, this.ship);
						}
						for (int i = 0; i < 10; i++) {
							target.splitShip();
						}
				...
				if(missiletarget != null){
					if(MathUtils.getDistance(slotabsloc,missiletarget.getLocation())<10f){
						while (missiletarget.getHullLevel()>0){
							engine.applyDamage(missiletarget, missiletarget.getLocation(), 100f, DamageType.FRAGMENTATION, 0, true, false, this.ship);
						}
						for (int i = 0; i < 10; i++) {
