...
public class omm_demolition_rocket_pod implements ProximityExplosionEffect {

    public void onExplosion(DamagingProjectileAPI explosion, DamagingProjectileAPI originalProjectile) {
        ArrayList<ShipAPI> fighters = new ArrayList<ShipAPI>();

        String variantId = "omm_demolition_rocket_pod_fighter_wing";
            ...
            Global.getCombatEngine().addPlugin(new BaseEveryFrameCombatPlugin() {
                ...
                public void advance(float amount, List<InputEventAPI> events) {
                    if (timer <= resistanceTime) {
                        fighter.getMutableStats().getHullDamageTakenMult().modifyMult("omm_demolition_rocket_pod", (timer * (0.5f / resistanceTime)) + 0.5f);
                        fighter.getMutableStats().getArmorDamageTakenMult().modifyMult("omm_demolition_rocket_pod", (timer * (0.5f / resistanceTime)) + 0.5f);
                        fighter.getMutableStats().getShieldDamageTakenMult().modifyMult("omm_demolition_rocket_pod", (timer * (0.5f / resistanceTime)) + 0.5f);
                    } else {
                        fighter.getMutableStats().getHullDamageTakenMult().unmodify("omm_demolition_rocket_pod");
                        fighter.getMutableStats().getArmorDamageTakenMult().unmodify("omm_demolition_rocket_pod");
                        fighter.getMutableStats().getShieldDamageTakenMult().unmodify("omm_demolition_rocket_pod");
                    }
