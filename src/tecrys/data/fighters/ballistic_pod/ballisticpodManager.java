...
//                        WeaponGroupAPI Group = FIGHTER.getWeaponGroupFor(weapon);
                        ...
                        WeaponGroupAPI drogroup = drone.getWeaponGroupFor(balwep);
                        WeaponGroupAPI balgroup = this.mothership.getWeaponGroupFor(wep);
                        WeaponGroupAPI lasgroup = drone.getWeaponGroupFor(laswep);


                        if (drogroup != null && balgroup != null)
                        { if (balgroup.isAutofiring() && !drogroup.isAutofiring()) {
...
//                                && (activegroup != null)
//                                && (Objects.equals(activegroup.getActiveWeapon().getId(), balwep.getId()))
                                ...
                                for (WeaponAPI weapon : activeWeapon) {
                                    if (balwep != null)   {
                                    if (Objects.equals(weapon.getId(), balwep.getId())) {
...

                                        {
                                            ...
                                            if (player == this.mothership && !drone.isLanding() && !drone.isLiftingOff() && dronewep.getSlot().getId().equals("omm_laser") && balwep != null
                                                ...
                                                // dronewep.getSprite().setHeight(balwep.getRange()*2);
                                                // dronewep.getSprite().setCenterY(balwep.getRange());
                                                laswep.setForceFireOneFrame(true);
                                                laswep.ensureClonedSpec();
                                                laswep.getSpec().setMaxRange(balwep.getSpec().getMaxRange() - ((balwep.getSpec().getMaxRange() / 100) * 20));
                                            }
                                        ...
                                        if (Mouse.isButtonDown(0) && !player.getFluxTracker().isOverloadedOrVenting() && (dronewep.getType() != MISSILE) && dronewep.getSlot().getId().equals("ballisticslot")
                                        ...
                                        if ((OMMSettings.missile_key == 0 && Mouse.isButtonDown(2) || Mouse.isButtonDown(0)) && !player.getFluxTracker().isOverloadedOrVenting() && (dronewep.getType() == MISSILE) && dronewep.getSlot().getId().equals("ballisticslot")) {
                                            balwep.setForceFireOneFrame(true);           //clicky left drone shooty
                                        } else if ((Keyboard.isKeyDown(OMMSettings.missile_key) || Mouse.isButtonDown(0)) && !player.getFluxTracker().isOverloadedOrVenting() && (dronewep.getType() == MISSILE) && dronewep.getSlot().getId().equals("ballisticslot")) {
                                            balwep.setForceFireOneFrame(true);
                                        }
                                    } else if (!Objects.equals(weapon.getId(), balwep.getId())) {
                                    }
                                }
                                }
                            }}
