...
public class smallpodManager implements AdvanceableListener {
    ...
    public smallpodManager(ShipAPI mothership) {
        this.mothership = mothership;

        for (FighterWingAPI wing : this.mothership.getAllWings()) {
            if (wing.getSpec().getId().equals("omm_smallpod_wing")) {
    ...
    public Vector2f getDesiredPosition(ShipAPI drone) {
        ...
        if (mothership.getHullSpec().getHullId().startsWith("omm_koura")) {
...
//                        WeaponGroupAPI Group = FIGHTER.getWeaponGroupFor(weapon);
                        ...
                        for (WeaponAPI w : this.mothership.getAllWeapons()) {
                            if (w.getSlot().getId().equals("smlhybridslot"))
                                wep = w;
                        }

                        WeaponGroupAPI smldrogroup = drone.getWeaponGroupFor(smlwep);
                        WeaponGroupAPI smlgroup = this.mothership.getWeaponGroupFor(wep);
                        WeaponGroupAPI lasgroup = drone.getWeaponGroupFor(laswep);

                        // Weapon group controls:
                        if (smldrogroup != null && smlgroup != null) {

                            if ((!smldrogroup.isAutofiring() && player != this.mothership) || (!smldrogroup.isAutofiring() && this.mothership.equals(player) && !engine.isUIAutopilotOn())) {
                                drone.giveCommand(ShipCommand.TOGGLE_AUTOFIRE, null, drone.getWeaponGroupsCopy().indexOf(smldrogroup));
...
//                                drone.giveCommand(ShipCommand.TOGGLE_AUTOFIRE, null, drone.getWeaponGroupsCopy().indexOf(group));
                            ...
                            if (smldrogroup.isAutofiring()
                                    && player == this.mothership && engine.isUIAutopilotOn() && smlgroup == activegroup
                            )
                            {
                                drone.giveCommand(ShipCommand.TOGGLE_AUTOFIRE, null, drone.getWeaponGroupsCopy().indexOf(smldrogroup));
                            }
                            else if (       (smlgroup.isAutofiring() && !smldrogroup.isAutofiring()
                                    && player == this.mothership && engine.isUIAutopilotOn() && smlgroup != activegroup)
                            )
                            {
                                drone.giveCommand(ShipCommand.TOGGLE_AUTOFIRE, null, drone.getWeaponGroupsCopy().indexOf(smldrogroup));
...
//                                && (activegroup != null)
//                                && (Objects.equals(activegroup.getActiveWeapon().getId(), smlwep.getId()))
//                                && !activegroup.isAutofiring()
                                ...
                                for (WeaponAPI weapon : activeWeapon) {
                                    ...
                                    if (Objects.equals(weapon.getId(), smlwep.getId()) && smlgroup == activegroup) {
                                        ...
                                        {                 // make sure player-selected weapon group has autofire turned off:
                                            ...
                                            if (player == this.mothership && !drone.isLanding() && !drone.isLiftingOff() && dronewep.getSlot().getId().equals("omm_laser") && smlwep != null
                                                ...
                                                laswep.getSpec().setMaxRange(smlwep.getSpec().getMaxRange() - ((smlwep.getSpec().getMaxRange() / 100) * 20));
                                            }
                                        ...
                                        if (Mouse.isButtonDown(0) && !player.getFluxTracker().isOverloadedOrVenting() && (dronewep.getType() != MISSILE) && dronewep.getSlot().getId().equals("smlhybridslot")
                                                && weapon.getSlot().getId().equals("smlhybridslot")) {
                                        ...
                                        if ((OMMSettings.missile_key == 0 && Mouse.isButtonDown(2) || Mouse.isButtonDown(0))&& !player.getFluxTracker().isOverloadedOrVenting() && (dronewep.getType() == MISSILE) && dronewep.getSlot().getId().equals("smlhybridslot")) {
                                            smlwep.setForceFireOneFrame(true);
                                        } else if ((Keyboard.isKeyDown(OMMSettings.missile_key) || Mouse.isButtonDown(0) )&& !player.getFluxTracker().isOverloadedOrVenting() && (dronewep.getType() == MISSILE) && dronewep.getSlot().getId().equals("smlhybridslot")) {
                                            smlwep.setForceFireOneFrame(true);
                                        }
                                    }
                                    else if (!Objects.equals(weapon.getId(), smlwep.getId())) {}
                                }
                            }
                            if(laswep != null)    {
                                laswep.setCurrAngle(smlwep.getCurrAngle());}       //aims the targeting laser towards cursor
                        }
            ...
            for (FighterWingAPI wing : wings) {
                ...
                for (int i = 0; i < wing.getWingMembers().size(); i++) {
                    ...
                    if (fighter.getAllWeapons().isEmpty())
                        continue;
                    if (!wing.getWingId().equals("omm_smallpod_wing")) {
                        continue;
                    }
                    ...
                    String str = (String) Global.getCombatEngine().getCustomData().get("omm_smlhybriddroneWeaponId" + this.mothership.getId());
                    if (str == null)
                        str = "No weapon";
                    ...
                    if (!fighter.getAllWeapons().get(0).getId().equals(str)) {
                        ...
                        if (ship.getVariant().getWeaponSpec("smlhybridslot") != null) {
                            Global.getCombatEngine().getCustomData().put("omm_smlhybriddroneWeaponId" + this.mothership.getId(), this.mothership.getVariant().getWeaponId("smlhybridslot"));
...
//                            wep.disable(true);
                            ...
                            stats.getVariant().clearSlot("smlhybridslot");
                            stats.getVariant().addWeapon("smlhybridslot", this.mothership.getVariant().getWeaponId("smlhybridslot"));
                            stats.getVariant().getWeaponSpec("smlhybridslot").addTag("FIRE_WHEN_INEFFICIENT");
